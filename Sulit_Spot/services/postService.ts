...
export type FeedCallback = (result: { data: Post[]; error: string | null }) => void;
export type PostCallback = (result: { data: Post | null; error: string | null }) => void;

export const subscribeToFeed = (callback: FeedCallback, category: 'All' | 'Food' | 'Item' | 'Tip' = 'All') => {
...
export const updatePost = async (postId: string, updates: Partial<Post>) => {
  ...
  const safeUpdates: Partial<Pick<Post, 'title' | 'description' | 'category' | 'priceRange' | 'locationText' | 'photoURL' | 'latitude' | 'longitude'>> = {};
  if (updates.title        !== undefined) safeUpdates.title        = updates.title;
  if (updates.description  !== undefined) safeUpdates.description  = updates.description;
  if (updates.category     !== undefined) safeUpdates.category     = updates.category;
  if (updates.priceRange   !== undefined) safeUpdates.priceRange   = updates.priceRange;
  if (updates.locationText !== undefined) safeUpdates.locationText = updates.locationText;
...
// ─────────────────────────────────────────────────────────────────────────────
export const voteAccuracy = async (postId: string, vote: 'accurate' | 'outdated') => {
  const user = auth.currentUser;
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (existing.exists()) throw new Error('You have already voted on this post.');
...
// ─────────────────────────────────────────────────────────────────────────────
export const getMyVote = async (postId: string): Promise<'accurate' | 'outdated' | null> => {
  const user = auth.currentUser;
  if (!user) return null;

  const snap = await getDoc(
    doc(db, 'confirmations', `${postId}_${user.uid}`)
  );
...
export const reportPost = async (postId: string, reason = '') => {
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (existing.exists()) throw new Error('You have already reported this post.');
...
export const hasReported = async (postId: string): Promise<boolean> => {
  ...
  const snap = await getDoc(
    doc(db, 'reports', `${postId}_${user.uid}`)
  );
  return snap.exists();
};
