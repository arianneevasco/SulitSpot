...
import type { Post, FeedCallback } from '../services/postService';
	...
	useEffect(() => {
		...
		const init = async () => {
			...
			try {
				...
				if (raw && isMounted) {
					const { data, timestamp, cachedCategory } = JSON.parse(raw) as CachePayload;
					const age = Date.now() - timestamp;
					if (age < CACHE_MAX_AGE && data.length > 0 && cachedCategory === category) {
						setRawPosts(data);
						setLoading(false);
					}
				}
